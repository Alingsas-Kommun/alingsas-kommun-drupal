    ...
    $.fn.termReferenceMenu = function (options) {
        ...
        return this.each(function () {
            ...
            var init = function () {
                ...
                //set behaviours
                $this.delegate("span.add", "click", function(){
                	addItem($(this).closest('li'));
                });
                $this.delegate("span.del", "click", function(){
                	...
                	$.each($this.itemsselected, function(index, val){
                		if(val.path.length > 0){
                			$.each(val.path, function(index2, val2){
                				$this.find('.container ul').append('<li name="' + val.tid + '"><span class="name">' + val2 + ' &gt; ' + val.name + '</span><span class="del">ta bort</span></li>');
                			});
                		}
                		else {
                			$this.find('.container ul').append('<li name="' + val.tid + '"><span class="name">' + val.name + '</span><span class="del">ta bort</span></li>');
                		}
                	});
                }
                    ...
                    if($.inArray(tid, $this.items) < 0) {
                    	...
                    	if($this.cache[tid].path.lenght > 0){
                    		$.each($this.cache[tid].path, function(index, val){
                    			$this.find('.container ul').append('<li name="' + tid + '"><span class="name">' + val + ' &gt; ' + $this.cache[tid].name + '</span><span class="del">ta bort</span></li>');
                    		});
                    	}
                    	else {
                    		$this.find('.container ul').append('<li name="' + tid + '"><span class="name">' + $this.cache[tid].name + '</span><span class="del">ta bort</span></li>');
                    	}
            ...
            var getItems = function( object ){
            	if(typeof object !== undefined && object.attr('name') !== undefined) {
            		...
            		voc = Drupal.settings[$this.attr('id').substr(5).replace(/-/g,'_')].voc;
            	  if($this.lock == 0) {
            		    if (typeof $this.cache[tid] !== "undefined" && $this.cache[tid].loaded === true) {
	                        ...
	                        object.find('ul').append('<li class="' + attributes + '" name="' + val.tid + '"><span class="name">' + val.name + '</span><span class="add">lägg till</span></li>');
	                      });
            		      ...
            		      $.getJSON('/term-reference-menu/' + tid + '/' + voc + '/get', function(data) {
            		        ...
            		        $.each(data, function( index, val) { 
            		        	...
            		        	if(val.name == 'Alingsås kommun') {
		                            attributes += ' alingsaskommun';
		                        }
            		        	object.find('ul').append('<li class="' + attributes + '" name="' + val.tid + '"><span class="name">' + val.name + '</span><span class="add">lägg till</span></li>');
            		        });
            		        object.find('ul').addClass("open").slideDown();
